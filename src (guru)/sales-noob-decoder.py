...
def try_to_decode(url):
    file_object = urllib2.urlopen(url)
    fp = StringIO.StringIO(file_object.read())

    img = Image.open(fp)
    img = img.convert('L')
    #img = img.point(lambda x: 0 if x<75 else 255, '1')
    img.save("result_captcha.jpg")
    
    img2 = Image.open("result_captcha.jpg")
    img2.load()
    ...
    return (pattern.sub('', result)).replace(' ', '')
...
def get_for_rent_property(agent_list_page):
    ...
    while(True):
        if 'want to make sure' in list_browser.page_source.encode('utf-8'):
            ...
            try:
                WebDriverWait(list_browser, 30).until(EC.presence_of_element_located((By.ID, "recaptcha_challenge_image")))
                captcha_div = list_browser.find_element_by_id("recaptcha_challenge_image")
                url = captcha_div.get_attribute('src')
                
                result = try_to_decode(url)
                if result:
                    print 'trying:', result
                else:
                    print 'trying: lets try another one...'
                    
                inputCaptcha = list_browser.find_element_by_id("recaptcha_response_field")
                try:
                    inputCaptcha.send_keys(result)
                except:
                    pass
                    
                list_browser.find_element_by_id("dCF_input_complete").click()
            except:
                pass
    ...
    for element in keep_these:
        ...
        while(True):
            if 'want to make sure' in temp_browser.page_source.encode('utf-8'):
                ...
                print 'need help with captcha'
                try:
                    WebDriverWait(temp_browser, 30).until(EC.presence_of_element_located((By.ID, "recaptcha_challenge_image")))
                    captcha_div = temp_browser.find_element_by_id("recaptcha_challenge_image")
                    url = captcha_div.get_attribute('src')
                    
                    result = try_to_decode(url)
                    print 'trying:', result
                    
                    inputCaptcha = temp_browser.find_element_by_id("recaptcha_response_field")
                    try:
                        inputCaptcha.send_keys(result)
                    except:
                        pass
                        
                    temp_browser.find_element_by_id("dCF_input_complete").click()
                except:
                    pass
